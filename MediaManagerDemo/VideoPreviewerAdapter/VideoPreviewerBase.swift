...
class VideoPreviewerBase: NSObject, DJIVideoFeedSourceListener, DJIVideoFeedListener, DJIVideoPreviewerFrameControlDelegate {
    ...
    class func adapterForLightbridge2() -> VideoPreviewerBase {
        let adapter = self.adapterWithDefaultSettings()
        adapter.isForLightbridge2 = true
        return adapter
    }

    class func adapterWith(videoPreviewer:DJIVideoPreviewer, videoFeed:DJIVideoFeed) -> VideoPreviewerBase {
        let adapter = VideoPreviewerBase()
    ...
    //TODO: open vs public? necessary?
    open func startLightbridgeListen() {
        print("called base class implementation of startLightbridgeListen, only call VideoPreviewerSDKAdapter.startLightbridgeListen()")
    }
    ...
    //TODO: open vs public? necessary?
    open func stopLightbridgeListen() {
        print("called base class implementation of stopLightbridgeListen, only call VideoPreviewerSDKAdapter.stopLightbridgeListen()")
    }
    ...
    func updateEncodeType() {
        ...
        if Self.isUsingLightbridge2(with: self.productName,
